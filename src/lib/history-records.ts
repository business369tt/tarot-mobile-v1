...
function mapListItem(record: HistoryListQueryRecord): HistoryListItem {
  ...
  return {
    ...
    categoryLabel: categoryMeta.labelZh,
    categoryDescription: categoryMeta.descriptionZh,
    cardsSummary: reading.cardsSnapshot.map((card) => ({
      id: card.id,
      name: getCardDisplayMeta(card.id).nameZh,
      orientation: getOrientationDisplayMeta(card.orientation).zh,
    })),
