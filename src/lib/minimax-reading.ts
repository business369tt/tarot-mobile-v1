...
function buildQualityRepairFocus(issueCodes: AiQualityIssueCode[]) {
  ...
  const descriptions = issueCodes.map((code) => {
    switch (code) {
      case "too_short":
        return "- Expand the content so the answer is complete, not skeletal.";
      case "not_traditional_chinese":
        return "- Rewrite in natural Traditional Chinese only.";
      case "contains_markdown":
      case "contains_json":
        return "- Return plain natural-language content only, with no markdown fences or JSON wrappers.";
      case "mentions_ai":
        return "- Remove any AI self-reference or assistant framing.";
      case "template_tone":
        return "- Remove template phrases and make the voice feel like a tarot reader, not a chatbot.";
      case "too_vague":
        return "- Replace vague wording with a clear answer and concrete interpretation.";
      case "question_detached":
        return "- Tie the answer directly back to the user's exact question.";
      case "directness_weak":
        return "- Make the opening sentence answer the question immediately.";
      case "tarot_anchor_missing":
        return "- Explicitly anchor the interpretation to the named cards, roles, and orientations.";
      case "guidance_count_invalid":
        return "- Provide exactly three actionable guidance items.";
      case "guidance_not_actionable":
        return "- Make the guidance concrete and actionable, not abstract encouragement.";
      case "duplicate_sections":
        return "- Remove repetition so each section says something distinct.";
      default:
        return "- Improve the answer quality.";
    }
  });

  return ["Quality gates that failed in the previous draft:", ...descriptions].join(
    "\n",
  );
}
...
function buildReadingQualityChecklist(cards: SelectedTarotCard[]) {
  const perCardRules = cards.map((card) => {
    ...
    return `- ${getCardReadingFieldKey(card.role)} must explicitly mention ${displayMeta.nameZh}、${roleMeta.labelZh}、${orientationMeta.zh}, then explain how that specific card affects the question.`;
  });

  return [
    "Quality gates you MUST satisfy:",
    "- Output natural Traditional Chinese only.",
    "- Sound like a real tarot reader, not an AI assistant or customer support agent.",
    "- In questionCore, answer the user's question directly in the first sentence.",
    "- Do not use template openings such as '整體來看', '首先', '其次', '最後', or '希望這能幫助你'.",
    "- concreteGuidance must contain exactly 3 specific actions.",
    "- No markdown fences, no JSON explanation, no extra labels outside the schema.",
    ...perCardRules,
  ].join("\n");
}
...
function buildFollowupQualityChecklist(cards: SelectedTarotCard[]) {
  ...
  return [
    "Quality gates you MUST satisfy:",
    "- Output natural Traditional Chinese only.",
    "- Answer the follow-up question directly in the first sentence.",
    "- Explicitly connect the answer to at least one card from this spread.",
    `- Available card anchors: ${cardRule}.`,
    "- No AI self-reference, no template tone, no markdown fences.",
    "- Keep the answer clear, specific, and grounded in tarot interpretation.",
  ].join("\n");
}
...
function buildReadingPrompt(args: {
  ...
  const system = [
    ...
    "不要輸出簡體中文、英文段落、markdown、code fence 或任何 JSON 以外的多餘文字。",
    "你可以保留塔羅的語感，但不能神化、嚇人、或做絕對命運式斷言。",
  ].join(" ");

  const user = [
    "請根據以下資訊，輸出一份可直接給使用者閱讀的塔羅解讀 JSON。",
...
function buildReadingRepairPrompt(args: {
  ...
  const system = [
    "你是塔羅解讀內容總編輯。",
    "你的任務是把一段格式混亂或語氣不佳的塔羅草稿，修復成有效 JSON。",
    "只能使用繁體中文，不能輸出任何 JSON 以外的內容。",
...
export async function generateTarotReadingWithMiniMax(args: {
  ...
  try {
    ...
    repairIssueCodes = ["contains_json"];
    logAiEvent(
